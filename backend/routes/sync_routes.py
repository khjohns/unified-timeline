...
_sync_repo: SyncMappingRepository | None = None
...
def list_mappings():
    ...
    try:
        ...
        mappings = sync_repo.list_sync_mappings(
            project_id=project_id, enabled_only=enabled_only
        )

        return jsonify({"mappings": [m.model_dump(mode="json") for m in mappings]})
...
def trigger_sync(mapping_id: str):
    ...
    try:
        ...
        thread = threading.Thread(
            target=_run_sync_background, args=(mapping_id, full_sync), daemon=True
        )
        thread.start()
...
def _run_sync_background(mapping_id: str, full_sync: bool):
    ...
    try:
        ...
        _emit_sync_event(
            mapping_id,
            "started",
            {"sync_mapping_id": mapping_id, "full_sync": full_sync},
        )
...
def update_task_filters(mapping_id: str):
    ...
    try:
        ...
        success = sync_repo.update_sync_mapping(
            mapping_id, {"task_filters": task_filters}
        )
        if not success:
            return jsonify({"error": "Update failed"}), 500
...
def get_filter_options(mapping_id: str):
    ...
    try:
        ...
        return jsonify({"types": sorted(list(types_set)), "total_tasks": len(tasks)})
...
def get_mapping_tasks(mapping_id: str):
    ...
    try:
        ...
        tasks = dalux_client.get_tasks(project_id=mapping.dalux_project_id, limit=limit)
...
def get_mapping_forms(mapping_id: str):
    ...
    try:
        ...
        forms = dalux_client.get_forms(project_id=mapping.dalux_project_id, limit=limit)

        return jsonify({"forms": forms, "total": len(forms)})
...
def validate_config():
    ...
    try:
            ...
            if dalux_client:
                try:
                    if dalux_client.health_check():
                        ...
                        project_ids = [
                            p.get("data", {}).get("projectId") for p in projects
                        ]
                        if dalux_project_id in project_ids:
                            dalux_ok = True
                        else:
                            errors.append(
                                f"Dalux project {dalux_project_id} not found or not accessible"
                            )
                    else:
        ...
        return jsonify(
            {"valid": False, "dalux_ok": False, "catenda_ok": False, "errors": [str(e)]}
        ), 500
...
def test_connection(mapping_id: str):
    ...
    try:
        ...
        return jsonify(
            {"dalux_ok": dalux_ok, "catenda_ok": catenda_ok, "errors": errors}
        )
