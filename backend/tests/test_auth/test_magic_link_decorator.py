...
from lib.auth import get_magic_link_manager, require_magic_link
...
class TestMagicLinkDecorator:
    ...
    def app(self):
        ...
        def protected_route():
            ...
            return jsonify(
                {"success": True, "sak_id": request.magic_link_data.get("sak_id")}
            )
    ...
    def valid_token(self):
        ...
        return manager.generate(
            sak_id="TEST-001", email="test@example.com", ttl_hours=1
        )
    ...
    def test_require_magic_link_with_invalid_token(self, client):
        ...
        response = client.get(
            "/protected", headers={"Authorization": "Bearer invalid-token-12345"}
        )
        assert response.status_code == 401
        data = response.get_json()
        assert data["success"] is False
    ...
    def test_require_magic_link_with_expired_token(self, client):
        ...
        token = manager.generate(
            sak_id="TEST-002", email="test@example.com", ttl_hours=-1
        )
