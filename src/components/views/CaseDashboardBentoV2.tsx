/**
 * CaseDashboardBentoV2 - Organisk bento-dashboard med fem nye funksjoner:
 *
 * 1. TrackNextStep: Kontekstuell neste-steg direkte på det relevante kortet
 * 2. TrackStepper: Mini prosessflyt-indikator (dots + linje) i hvert kort
 * 3. CrossTrackActivity: Siste aktivitet på tvers av alle spor (under kortene)
 * 4. DependencyIndicator: Visuell kobling mellom master → dependent kort
 * 5. Metadata integrert i header via BentoHeaderMeta (i CasePageBento)
 *
 * Bygger på CaseDashboardBento men erstatter den separat StatusAlert-banneren
 * med kontekstuelle neste-steg-indikatorer per kort.
 */
...
// Bento-specific components
import { TrackStepper } from '../bento/TrackStepper';
import { TrackNextStep } from '../bento/TrackNextStep';
import { CrossTrackActivity } from '../bento/CrossTrackActivity';
import { DependencyIndicator } from '../bento/TrackConnector';
...
interface CaseDashboardBentoV2Props {
  state: SakState;
  userRole: 'TE' | 'BH';
...
export function CaseDashboardBentoV2({
  state,
  userRole,
  actions,
  grunnlagActions,
  ...
  // Grunnlag is active (sent or beyond) - for visual connectors
  const grunnlagActive = !grunnlagIkkeSendt;
  ...
  return (
    <section aria-labelledby="dashboard-heading">
      ...
      {/* Track cards grid with relative positioning for connectors */}
