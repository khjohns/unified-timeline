/**
 * SendFristForm Component
 *
 * Reusable form for submitting a new frist (deadline extension) claim.
 * Extracted from SendFristModal for use in both Modal and TrackFormView contexts.
...
export interface SendFristFormProps {
  sakId: string;
...
export function SendFristForm({
  ...
  useEffect(() => {
    if (hasBackup && !isDirty && !hasCheckedBackup.current) {
  ...
  // §33.4: Noytralt varsel skal sendes "uten ugrunnet opphold" - typisk 7-14 dager
  ...
  const onSubmit = (data: FristFormData) => {
    ...
    mutation.mutate({
      ...
      data: {
        ...
        // Metadata for tracking if this was response to BH foresporsel (§33.6.2)
  ...
  return (
    <>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 sm:space-y-6">
        {/* BH foresporsel warning (§33.6.2) - CRITICAL */}
        {harMottattForesporsel && (
          <Alert variant="danger" title="Svar på byggherrens foresporsel (§33.6.2)">
            <p>
                ...
                <>
                  <RadioGroup value={field.value} onValueChange={field.onChange} data-testid="frist-varsel-type">
                    {FRIST_VARSELTYPE_OPTIONS
                      ...
                      // §33.6.2 bokstav b er kun tilgjengelig som svar på foresporsel
                    ...
                    <div className="mt-4 space-y-3 sm:space-y-4">
                      ...
                      {field.value === 'begrunnelse_utsatt' && (
                        <p className="text-sm text-pkt-text-body-subtle mb-3">
                          <ExpandableText preview="Byggherren kan ettersporre spesifisert krav.">
                            Mottar totalentreprenoren foresporsel om å spesifisere fristkrav (§33.6.2), må han uten
                            ugrunnet opphold enten angi og begrunne antall dager, eller begrunne hvorfor
                            beregningsgrunnlag ikke foreligger. Gjor han ingen av delene i tide, tapes kravet.
                          </ExpandableText>
                        </p>
                      )}
                    </div>
        ...
        {/* Varsel om fristforlengelse (§33.4) - for both varsel and spesifisert */}
        {(selectedVarselType === 'varsel' || selectedVarselType === 'spesifisert') && (
          <SectionContainer title="Varsel om fristforlengelse (§33.4)">
            <div className="space-y-3 sm:space-y-4">
              ...
              {/* §33.4 Preklusjonsvarsel - kun for noytralt varsel */}
              {selectedVarselType === 'varsel' && erNoytraltVarselSent && (
                ...
                >
                  ...
                  {erNoytraltVarselKritisk
                    ...
                    : ' Du bor varsle snarest for å bevare retten til fristforlengelse.'}
                </Alert>
              )}
                  ...
                  {harTidligereVarslet ? (
                    <>
                      <FormField
                        ...
                        helpText="Skriftlig varsel, e-post til avtalt adresse, eller innfort i referat (§5)."
                      ...
                      >
                        <div className="space-y-2">
                          ...
                          <Checkbox
                            ...
                            label="Byggemote (innfort i referat)"
                            value="byggemoete"
                            {...register('frist_varsel_metoder')}
                          />
                        </div>
        ...
        {selectedVarselType === 'begrunnelse_utsatt' && (
          ...
          >
            <Alert variant="info" title="Konsekvens av §33.6.2 bokstav b">
              ...
              «uten ugrunnet opphold» når grunnlaget foreligger. Byggherren kan sende ny foresporsel senere.
            </Alert>
          </SectionContainer>
        )}
          ...
          <SectionContainer title="Krav og begrunnelse">
            <div className="space-y-3 sm:space-y-4">
              ...
              {grunnlagEvent?.hovedkategori && erForceMajeure(grunnlagEvent.hovedkategori) ? (
                <p className="text-sm text-pkt-text-body-subtle">
                  <ExpandableText preview="Totalentreprenoren har krav på fristforlengelse ved force majeure.">
                    Dersom fremdriften hindres av ekstraordinære og upåregnelige forhold utenfor partens
                    kontroll (force majeure), har totalentreprenoren krav på fristforlengelse (§33.3).
                  </ExpandableText>
                </p>
              ) : (
                <p className="text-sm text-pkt-text-body-subtle">
                  <ExpandableText preview="Totalentreprenoren har krav på fristforlengelse når fremdriften hindres av byggherrens forhold.">
                    Dersom fremdriften hindres på grunn av endringer, forsinkelse eller svikt i byggherrens
                    medvirkning, eller andre forhold byggherren bærer risikoen for, har totalentreprenoren
              ...
              <p className="text-sm text-pkt-text-body-subtle">
                <ExpandableText preview="Fristforlengelsen skal svare til den virkning hindringen har hatt for fremdriften.">
                  ...
                  Ved beregningen skal det tas hensyn til nodvendig avbrudd og oppstart, årstidsforskyvning,
                  den samlede virkning av tidligere fristforlengelser, og om entreprenoren har oppfylt sin
                  tapsbegrensningsplikt. Forlengelsen skal ikke overstige det som er nodvendig for å kompensere
