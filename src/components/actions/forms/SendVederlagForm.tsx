/**
 * SendVederlagForm Component
 *
 * Reusable form for submitting a new vederlag (compensation) claim.
 * Extracted from SendVederlagModal for use in both Modal and TrackFormView contexts.
...
import { VederlagMethodSelector } from '../shared';
...
const vederlagSchema = z.object({
  ...
  // Saerskilte krav (§34.1.3) - Rigg/Drift og Produktivitet
...
export interface SendVederlagFormProps {
  sakId: string;
...
export function SendVederlagForm({
  sakId,
  grunnlagEventId,
  grunnlagEvent,
  onSuccess,
  ...
  // Check preclusion for rigg/drift (§34.1.3 forste ledd) - 7 days threshold
  ...
  const onSubmit = (data: VederlagFormData) => {
    ...
    // Build saerskilt krav structure with separate dates per §34.1.3
  ...
  return (
    <>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 sm:space-y-6">
        ...
        <SectionContainer title="Kravets omfang">
          ...
          {selectedMetode === 'REGNINGSARBEID' && (
            <>
              <Alert variant="info" className="mb-3">
                Ved regningsarbeid faktureres kostnadene lopende. Ved fradrag reduseres vederlaget
          ...
          {selectedMetode === 'FASTPRIS_TILBUD' && (
            <FormField
              ...
              required
              error={errors.belop_direkte?.message}
              helpText="Spesifisert tilbud (§34.2.1). Ved avslag faller oppgjoret tilbake på enhetspriser (§34.3) eller regningsarbeid (§34.4)."
            >
              <Controller
                name="belop_direkte"
                control={control}
        ...
        {/* 3. Saerskilte krav (§34.1.3) - Rigg, Drift, Produktivitet */}
        <SectionContainer
          title="Saerskilte krav (§34.1.3)"
          description="Krav om okte rigg-/driftskostnader og produktivitetstap krever saerskilt varsel"
          optional
        >

          {/* Rigg/Drift section (§34.1.3 forste ledd) */}
          <div className="mb-4">
            <Controller
              name="har_rigg_krav"
              control={control}
              render={({ field }) => (
                <Checkbox
                  id="har_rigg_krav"
                  label="Okte rigg- og driftsutgifter"
                  checked={field.value}
              ...
              <div className="mt-3 ml-6 space-y-3 sm:space-y-4 border-l-2 border-pkt-border-subtle pl-4">
                <p className="text-sm text-pkt-text-body-subtle">
                  <ExpandableText preview="Kravet må varsles «uten ugrunnet opphold».">
                    Krav på saerskilt justering for rigg/drift må varsles «uten ugrunnet opphold» etter at entreprenoren ble klar over at utgifter ville pålope som en nodvendig folge av endring, forsinkelse/svikt ved byggherrens ytelser, eller andre forhold byggherren har risikoen for.
                  </ExpandableText>
                </p>

                <FormField
                  label="Estimert belop for rigg/drift"
                ...
                <FormField
                  ...
                  helpText="Varslingsfristen loper fra dette tidspunktet"
                >
                  <Controller
          ...
          <div>
            ...
            {harProduktivitetKrav && (
              <div className="mt-3 ml-6 space-y-3 sm:space-y-4 border-l-2 border-pkt-border-subtle pl-4">
                <p className="text-sm text-pkt-text-body-subtle">
                  <ExpandableText preview="Kravet må varsles «uten ugrunnet opphold».">
                    Krav på produktivitetstap må varsles «uten ugrunnet opphold» etter at entreprenoren ble klar over at utgifter ville pålope som folge av endring, forsinkelse/svikt ved byggherrens ytelser, eller andre forhold byggherren har risikoen for.
                  </ExpandableText>
                </p>

                <FormField
                  label="Estimert belop for produktivitetstap"
