/**
 * SendGrunnlagForm Component
 *
 * Reusable form for submitting a new grunnlag (basis/foundation) claim,
 * or updating an existing one (when originalEvent is provided).
 * Extracted from SendGrunnlagModal for use in both Modal and TrackFormView contexts.
 *
 * MODES:
...
export interface SendGrunnlagFormProps {
  sakId: string;
  /** Called after successful submission (modal closes, or card collapses) */
  onSuccess: () => void;
  /** Called when user clicks Avbryt */
  onCancel: () => void;
  /** Callback when Catenda sync was skipped or failed */
  onCatendaWarning?: () => void;
  /** UPDATE MODE: When provided, form operates in update mode */
  originalEvent?: {
    event_id: string;
...
export function SendGrunnlagForm({
  sakId,
  onSuccess,
  onCancel,
  onCatendaWarning,
  originalEvent,
}: SendGrunnlagFormProps) {
  ...
  // Reset form when mounting in update mode with originalEvent
  // (replaces the open-based reset; form mounts/unmounts with the modal)
  useEffect(() => {
  ...
  }, [isUpdateMode, originalEvent, reset, computedDefaultValues]);
